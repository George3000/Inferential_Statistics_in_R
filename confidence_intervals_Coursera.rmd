...
```{r set-seed}
set.seed(50)                 # make sure to change the seed
```
...
str(ames)
hist(ames$area, breaks = 25)
ggplot(aes(x=area), data = ames) +geom_histogram(binwidth = 250)
samp %>%
  summarise(x_bar = mean(area), x_med = median(area), 
            s = sd(area), x_iqr = IQR(area),
            x_min = min(area), x_max = max(area),
            x_q1 = quantile(area, 0.25),  # first quartile, 25th percentile
            x_q3 = quantile(area, 0.75))  # third quartile, 75th percentile
...
```{r plot-99-perc-cis}
# type your code for the Exercise here, and Knit
CI_99 <- c(lower = mean(samp$area) - Z_star_99 * (sd(samp$area) / sqrt(n)),mean(samp$area) + Z_star_99 * (sd(samp$area) / sqrt(n)))
